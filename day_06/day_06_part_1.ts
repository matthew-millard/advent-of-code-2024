...
const grid: string[][] = await readFile('../inputs/day_06.txt', 'utf-8').then(value =>
  value
    .trim()
    .split('\n')
    .map(row => row.split(''))
...
let currentPositionOfGuard: number[] = getStartingPositionOfGuard(grid);
...
function getStartingPositionOfGuard(grid: string[][]) {
  ...
  for (let rowIndex = 0; rowIndex < numOfRows; rowIndex++) {
    ...
    if (foundGuardPosition) break;
  }

  return [positionX, positionY];
}

function outOfBounds([row, column]: number[]) {
  return row < 0 || row >= numOfRows || column < 0 || column >= numOfCols;
}

function getCharAtPosition([row, column]: number[]) {
