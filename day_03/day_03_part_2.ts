...
let total = 0;
let isMulEnabled: boolean = true; // default true
...
const mulMatches: Matches = [...corruptedMemory.matchAll(mulRegExp)].map(match => ({ type: 'mul', match }));
const doMatches: Matches = [...corruptedMemory.matchAll(doRegExp)].map(match => ({ type: 'do', match }));
const dontMatches: Matches = [...corruptedMemory.matchAll(dontRegExp)].map(match => ({ type: "don't", match }));
